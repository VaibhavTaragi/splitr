...
export const spendingInsights = inngest.createFunction(
  ...
  async ({ step }) => {
    /* ─── 1. Pull users with expenses in the lookback window ─────────── */
    ...
    if (!users?.length) {
      return {
        ...
        failed: 0,
        message:
          "No users with expenses in the last 60 days. Add expenses or run seed data.",
      };
    }
...
5. Recommendations for Next Month
      ...
      try {
        ...
        const toEmail = testMode ? process.env.RESEND_TEST_EMAIL : user.email;
        const emailResult = await step.run(`Email · ${user._id}`, () =>
          convex.action(api.email.sendEmail, {
            to: toEmail,
            subject: "Your Monthly Spending Insights",
            html: `
              <h1>Your Monthly Financial Insights</h1>
              ${testMode ? `<p><em>Test mode: would have been sent to ${user.email}</em></p>` : ""}
              <p>Hi ${user.name},</p>

...
// Number of days to look back for "monthly" insights (wider window so more users get insights)
const INSIGHTS_LOOKBACK_DAYS = 60;
...
export const getUsersWithExpenses = query({
  handler: async (ctx) => {
    ...
    // Look back N days so we include recent expenses (avoids empty results from strict "last calendar month")
    const now = Date.now();
    const monthStart = now - INSIGHTS_LOOKBACK_DAYS * 24 * 60 * 60 * 1000;
...
// Get a specific user's expenses for the insight window (same as getUsersWithExpenses)
